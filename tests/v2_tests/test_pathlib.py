...
def test_path_glob(storage: str, pattern: str) -> None:
    with from_url(url=storage) as fs:
        ...
        if isinstance(fs, Local):
            expected = pathlib.Path(storage)
            expected_entries = []
            for entry in expected.glob(pattern):
                if not entry.is_dir():
                    expected_entries.append(str(entry.relative_to(storage)))
            assert sorted(actual_entries) == sorted(expected_entries)
...
def test_path_rglob(storage: str, pattern: str) -> None:
    with from_url(url=storage) as fs:
        ...
        if isinstance(fs, Local):
            expected = pathlib.Path(storage)
            expected_entries = []
            for entry in expected.rglob(pattern):
                if not entry.is_dir():
                    item = _removeprefix(str(entry), storage + "/")
                    expected_entries.append(item)
            assert sorted(actual_entries) == sorted(expected_entries)
...
def test_path_iterdir(storage: str, path: str) -> None:
    with from_url(url=storage) as fs:
        ...
        if isinstance(fs, Local):
            expected = pathlib.Path(storage, path)
            expected_entries = []
