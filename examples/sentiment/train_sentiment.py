...
def main():
    ...
    train_data = [convert_tree(vocab, tree)
                  for tree in data.read_corpus(os.path.join(args.root + 'trees/train.txt'), max_size)]
    train_iter = chainer.iterators.SerialIterator(train_data, batchsize)
    validation_data = [convert_tree(vocab, tree)
                       for tree in data.read_corpus(os.path.join(args.root + 'trees/dev.txt'), max_size)]
    validation_iter = chainer.iterators.SerialIterator(
        validation_data, batchsize, repeat=False, shuffle=False)
    test_data = [convert_tree(vocab, tree)
                 for tree in data.read_corpus(os.path.join(args.root + 'trees/test.txt'), max_size)]
